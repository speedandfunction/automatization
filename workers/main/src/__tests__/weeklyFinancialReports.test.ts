...
describe('weeklyFinancialReportsWorkflow', () => {
  ...
  it('should log and rethrow errors', async () => {
    const logSpy = vi.spyOn(utils, 'logWorkflowError').mockImplementation(() => {});
    const originalToLocaleString = Number.prototype.toLocaleString;
    Number.prototype.toLocaleString = () => { throw new Error('Test error'); };

    await expect(weeklyFinancialReportsWorkflow()).rejects.toThrow('Test error');
    expect(logSpy).toHaveBeenCalledWith('Weekly Financial Reports', expect.any(Error));
